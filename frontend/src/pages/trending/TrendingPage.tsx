...
const TrendingPage = () => {
	const { fetchTrendingSongs, trendingSongs, isLoading } = useMusicStore();
	...
	console.log("Trending Songs:", trendingSongs.length, trendingSongs);
		...
		<main className='rounded-md overflow-hidden h-full bg-gradient-to-b from-zinc-800 to-zinc-900'>
			...
			<ScrollArea className='h-[calc(100vh-180px)]'>
				<div className='p-4 sm:p-6'>
					...
					<div className='mb-6'>
						<h1 className='text-3xl sm:text-4xl font-bold mb-2'>Trending</h1>
						<p className='text-zinc-400'>
							{trendingSongs.length} {trendingSongs.length === 1 ? 'song' : 'songs'}
						</p>
					</div>
					...
					{!isLoading && trendingSongs.length === 0 && (
						<div className='text-center py-12'>
							<p className='text-zinc-400 text-lg'>No trending songs available yet</p>
						</div>
					)}
				</div>
			</ScrollArea>
